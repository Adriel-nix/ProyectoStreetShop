...
fetch("https://raw.githubusercontent.com/Adriel-nix/ProyectoStreetShop/master/datos/clothes.json")
  .then(res => res.json())
  .then(data => {
    const lista = document.getElementById("resumenCarrito2");
    const productos = data.clothes.filter(item => carrito.includes(item.Id));
    ...
    document.getElementById("totalPrecio2").textContent = `₡ ${total.toLocaleString("es-CR")}`;
  });
...
document.getElementById("formCheckout").addEventListener("submit", function (e) {
  ...
  let vendidos = JSON.parse(localStorage.getItem("vendidos2")) || [];
vendidos = [...new Set([...vendidos, ...carrito])]; // Evita duplicados
localStorage.setItem("vendidos2", JSON.stringify(vendidos));
